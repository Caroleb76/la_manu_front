...
export default function ContractCreateForm({
    ...
    async function loadData() {
      ...
      if (contractResp) {
        ...
        reset({
          ...
          startDate:
            convertDateToFranceTimeZone(contractResp.data.startDate) || "",
          endDate: convertDateToFranceTimeZone(contractResp.data.endDate) || "",
  ...
  useEffect(() => {
    const getSessionData = async () => {
      const response =
        await sessionFormationsHelper.getSessionById(currentSessionId);
      if (response) {
  ...
  function extractData(data) {
    try {
      const { lastName, firstName, address, postalCode, city, ...otherFields } =
        data;
    ...
    <div className={styles.borderPopup}>
      ...
      <form action="">
                ...
                {formateurs &&
                  formateurs.map((formateur) => (
                    <option value={formateur.id} key={formateur.id}>
                      {formateur.lastName} {formateur.firstName}
                    </option>
                  ))}
              </InputSelect>
                ...
                {sessionsFormation &&
                  sessionsFormation.map((session) => (
                    <option value={session.id} key={session.id}>
                      {" "}
                      {session.Formation.name} - {session.serialNumber}
                    </option>
                  ))}
              </InputSelect>
            ...
            {currentInterventions &&
              currentInterventions.map((intervention, index) => (
                <tr key={index}>
                  ...
                  <td>
                    {convertDateToFranceTimeZone(intervention.dateIntervention)}
                  </td>
                  <td>
                  ...
                  <td>{intervention.InterventionCategory?.name ?? "N/A"}</td>
                  <td>
        ...
        <div className={styles.popupButtons}>
          {contractId && (
            <h3 className={isSigned ? styles.isSigned : styles.isNotSigned}>
              {isSigned ? "Contrat déjà signé" : "Contrat à signer"}
            </h3>
          )}
