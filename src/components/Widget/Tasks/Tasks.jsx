...
export default function Tasks() {
  ...
  useEffect(() => {
    const getUnsignedContracts = async () => {
      const response = await contractsHelper.getContractsByUserId(user.id);
      if (response) {
      ...
      <ul className={styles.list}>
        {contracts?.map((contract) => (
          <li className="hover-list" key={contract.id}>
            <Link to={`/dashboard/contracts/sign/${contract.id}`}>
              {" "}
              {contract.SessionFormation.Formation.name} - Du{" "}
              {convertDateToFranceTimeZone(contract.startDate)} au{" "}
              {convertDateToFranceTimeZone(contract.endDate)}
            </Link>
          </li>
        ))}
      </ul>
