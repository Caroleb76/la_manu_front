...
export default function InterventionCard({
  ...
  return (
    <article className={styles.card}>
      ...
      {confirmValidation && (
        ...
        >
          <div className={styles.validatePopup__wrapper}>
            <h1 className={styles.validatePopup__title}>
              Etes vous certain de vouloir valider cette intervention ?
            </h1>
            <p>
              Veillez à bien renseigner les frais de déplacement avant de
              valider
            </p>
            <div className={styles.validatePopup__btnWrap}>
              <button className="btn btn-success btn-sm" onClick={validate}>
                Valider
              </button>
              <button
        ...
        <div className={styles.cardRight}>
          ...
          <div className={styles.lowerRightCard}>
            ...
            <button
              type="button"
              className={`btn btn-primary ${
                iv.validatedByFormateur || disableActions ? "btn-disabled" : ""
              }`}
      ...
      {iv.description && <p className={styles.description}>{iv.description}</p>}
    </article>
  );
}
