...
async function getSessionById(sessionId) {
  const response = await apiClient(`SessionFormation/${sessionId}`, {
    method: "GET",
  });
  return response;
}
...
async function createSessionFormation(sessionFormation) {
  const response = await apiClient("SessionFormation/", {
    method: "POST",
...
async function updateSessionFormation(sessionFormation) {
  const response = await apiClient(`SessionFormation/${sessionFormation.id}`, {
    method: "PUT",
    body: sessionFormation,
  });
  return response;
