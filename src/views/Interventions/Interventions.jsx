...
export default function Interventions() {
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            try {
                const res = await formationHelper.allFormationByFormateurId(
                    user.id
                );
                if (ignore) return;
    ...
    const { validated, coming, pending } = useMemo(() => {
        ...
        coming.sort(
            (a, b) =>
                new Date(a.dateIntervention) - new Date(b.dateIntervention)
        );
        
        validated.sort(
            (a, b) =>
                new Date(b.dateIntervention) - new Date(a.dateIntervention)
        );
